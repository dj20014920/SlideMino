...
// 스킨 미리보기 타일 렌더링
const SkinPreviewTile = React.memo<{ value: number; skinHex: string; tilePx: number }>(
  ({ value, skinHex, tilePx }) => {
    const baseHex = getSkinColorForValue(value, skinHex);
    const { backgroundImage, baseRgb } = buildGradient(baseHex);
    const textStyle = getWhiteTextStyleForBackground(baseRgb);
    const { text, fontPx } = getTileNumberLayout(value, tilePx);

    return (
      <div
        className="rounded-2xl flex items-center justify-center font-semibold overflow-hidden text-center select-none shrink-0"
        style={{
          width: `${tilePx}px`,
          height: `${tilePx}px`,
          fontSize: `${fontPx}px`,
...
export function SkinModal({ open, onClose }: SkinModalProps) {
  ...
  const [acquisitionSkin, setAcquisitionSkin] = useState<{ hex: string } | null>(null);
  ...
  // 미리보기 표시할 색상: 선택된 스킨 > 활성 스킨 > 첫 번째 카탈로그
  const previewHex = selectedSkinHex ?? activeSkin?.hex ?? SKIN_CATALOG[0]?.hex ?? '#64748b';
          ...
          <div className="p-5 overflow-y-auto min-h-0 flex-1 space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-mono text-gray-500">{previewHex.toUpperCase()}</span>
              </div>
              <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">
                {SKIN_PREVIEW_VALUES.map((v) => (
                  <SkinPreviewTile key={v} value={v} skinHex={previewHex} tilePx={52} />
                ))}
              </div>
              {/* 선택 상태 안내 */}
              {selectedSkinId && (
